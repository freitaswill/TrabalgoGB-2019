...
void InputTexto::atualizar()
{
	...
	if (trocaString == true)
	{
		txtSenha.setString(gTeclado.inputTexto.getString());
	}
	else
	{
...
void InputTexto::desenharSenha()
{
	...
	txtSenha.desenhar(gJanela.getLargura() / 2, gJanela.getAltura() / 2 + 120);
	...
	if (tamSelecao != 0)
	{
		Quad retan;
		retan.larg = 0;
		retan.alt = txtSenha.getFonte()->getAlturaGlifos();
		retan.x = (gJanela.getLargura() / 2) - txtSenha.getLargura() / 2;
		...
		for (int i = inicio; i < fim; ++i)
			retan.larg += txtSenha.getLetra(i)->glifo->avanco*txt.getEscalaX();

		for (int i = 0; i < inicio; ++i)
			retan.x += txtSenha.getLetra(i)->glifo->avanco*txt.getEscalaX();

		gGraficos.desenharRetangulo(retan, 30, 200, 230);
	}

	//Desenhar o cursor do texto
	int altCursor = txtSenha.getFonte()->getAlturaGlifos();
	int xCursor = (gJanela.getLargura() / 2) - txtSenha.getLargura() / 2;
	int yCursor = (gJanela.getAltura() / 2 + 120) - altCursor / 2;
	int pos = gTeclado.inputTexto.getPosCursor();
	for (int i = 0; i < pos; ++i)
	xCursor += txtSenha.getLetra(i)->glifo->avanco*txt.getEscalaX();
...
{
	//4) Mostrar o texto na tela
	txt.desenhar(gJanela.getLargura() / 2, gJanela.getAltura() / 2 - 20);
